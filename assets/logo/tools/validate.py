...
# If LFS enforcing is enabled, any PNG in drafts/** above this threshold
# must be LFS-tracked.
...
def check_processed_pairs_and_sizes():
    if not PROCESSED.exists():
        print(
            "No processed dir found; skipping processed checks "
            "(nothing to validate)."
        )
        return
...
def check_lfs_coverage_for_drafts():
    ...
    if not ENFORCE_LFS:
        print(
            "LFS enforcement is OFF (set MANTRA_LOGO_ENFORCE_LFS=1 to enable)."
        )
        return
    ...
    if violations:
        ...
        print("::endgroup::")
        fail(
            "Some large PNGs under assets/logo/drafts/** are not tracked by "
            "Git LFS. Enable LFS for these files or disable enforcement for "
            "this run."
        )
    else:
